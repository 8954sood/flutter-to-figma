...
| Visibility | `Visibility(maintainSize: true)` — 레이아웃 공간 유지 + opacity 0 |
...
flutter-to-figma/
├── tools/
│   ├── crawler/                            # Dart 크롤러 소스 모듈 (편집 대상)
│   ├── test_flutter/                       # Flutter 크롤러 통합 테스트
│   ├── merge.dart                          # 빌드 스크립트
│   ├── export_figma_layout.dart            # 추출기 템플릿
│   ├── crawler_source.dart                 # 생성 파일 (편집 금지)
│   └── generated_export_figma_layout.dart  # 생성 파일 (편집 금지)
├── figma_plugin/
│   ├── src/                                # JS 플러그인 소스 모듈 (편집 대상)
│   ├── test/                               # 전처리 파이프라인 단위 + 통합 테스트
│   ├── manifest.json                       # Figma 플러그인 매니페스트
│   ├── code.js                             # 생성 파일 (편집 금지)
│   └── ui.html                             # 플러그인 UI (JSON 입력 + 갤러리)
├── README.md
├── README.ko.md
├── CHANGELOG.md
├── LICENSE
