...
exports.initDumpFullDesign = initDumpFullDesign;
exports.dumpFullDesign = dumpFullDesign;
 ...
 */
function initDumpFullDesign(outputChannel) {
    output = outputChannel;
}
/**
 * 메인 커맨드: 전체 화면 디자인 덤프
 */
async function dumpFullDesign() {
    try {
        ...
        vscode.window.showInformationMessage(`Flutter 화면 디자인 정보를 JSON으로 덤프했습니다: ${dumpDir}`);
    }
    catch (err) {
 ...
 */
async function dumpFullDesignFromVm(vmServiceUri, dumpDir) {
    ...
    return new Promise((resolve, reject) => {
        ...
        ws.on('open', async () => {
            try {
                ...
                // inspector 초기화
                try {
                    await (0, vmService_1.sendRequest)('ext.flutter.inspector.setSelectionById', {
                        id: '0',
                    ...
                    output.appendLine(`[Inspector] setSelectionById(0) 오류 (무시 가능): ${e?.message ?? String(e)}`);
                }
                try {
                    await (0, vmService_1.sendRequest)('ext.flutter.inspector.getSelectedWidget', {
                        objectGroup: 'vscode-inspector',
                        isolateId,
                    });
                }
                catch (e) {
                    output.appendLine(`[Inspector] getSelectedWidget 오류 (무시 가능): ${e?.message ?? String(e)}`);
                }
                ...
                output.appendLine(`[DUMP] 총 ${valueIds.length}개 diagnostics valueId 수집`);
                const layoutById = {};
                const detailsById = {};
                ...
                for (const id of valueIds) {
                    ...
                    // LayoutExplorerNode
                    try {
                        const layoutNode = await (0, vmService_1.sendRequest)('ext.flutter.inspector.getLayoutExplorerNode', {
                            id,
                        ...
                        output.appendLine(`[WARN] getLayoutExplorerNode(${id}) 실패: ${e?.message ?? String(e)}`);
                    }
                    // DetailsSubtree
                    try {
                        const detailsNode = await (0, vmService_1.sendRequest)('ext.flutter.inspector.getDetailsSubtree', {
                            objectGroup: 'vscode-inspector',
                            arg: id,
                            subtreeDepth: '2',
                            isolateId,
                        ...
                        output.appendLine(`[WARN] getDetailsSubtree(${id}) 실패: ${e?.message ?? String(e)}`);
                    }
 ...
 */
async function waitForRootWidgetSummaryTree(isolateId) {
    const maxAttempts = 20;
    const delayMs = 500;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            output.appendLine(`[Inspector] getRootWidgetSummaryTree 시도 ${attempt}/${maxAttempts}`);
            const tree = await (0, vmService_1.sendRequest)('ext.flutter.inspector.getRootWidgetSummaryTree', {
                objectGroup: 'vscode-inspector',
                isSummaryTree: true,
                withPreviews: true,
                isolateId,
            ...
            if (msg.includes('Null check operator used on a null value')) {
                output.appendLine(`[Inspector] widget tree not ready yet (Null check). ${delayMs}ms 후 재시도...`);
                await (0, utils_1.sleep)(delayMs);
                continue;
            }
            output.appendLine(`[Inspector] getRootWidgetSummaryTree 치명적 오류: ${msg}`);
            throw e;
        }
    }
    throw new Error('Widget tree not ready after multiple attempts (getRootWidgetSummaryTree 계속 실패)');
}
/**
 * 요약 트리에서 모든 diagnostics valueId 수집
 */
function collectAllValueIds(tree) {
    const ids = new Set();
    function dfs(node) {
...
//# sourceMappingURL=dumpFullDesign.js.map